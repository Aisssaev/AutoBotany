package net.neolab.autobotany.events;

import net.neolab.autobotany.AutoBotany;
import net.neolab.autobotany.ModItems;
import net.neolab.autobotany.util.NumberFormatter;
...
import static net.neolab.autobotany.ModBlocks.*;

@Mod.EventBusSubscriber(modid = AutoBotany.MOD_ID, bus = Mod.EventBusSubscriber.Bus.FORGE)
        ...
        if (!stack.isEmpty()){
            ...
            if (tooltipManaAllBlocks.contains(stack.getItem()) && stack.hasTag()){
                if (tag.getCompound("BlockEntityTag").contains("mana")){
                    ...
                    event.getToolTip().add(Component.translatable("autobotany.tooltip.item.mana", NumberFormatter.formatInteger(mana)));
                }
            }
                ...
                if (tag.getCompound("BlockEntityTag").contains("fluid")){
                    ...
                    event.getToolTip().add(Component.translatable("autobotany.tooltip.item.water", NumberFormatter.formatIntegerWater(water)));
                }
            }
        }
